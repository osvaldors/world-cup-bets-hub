...
export default function SimCupStandingsPage() {
  const { cupGroupStandings, mergedMatches, mergedBets, participants } = useSimulator();
  ...
  // Get group stage matches organized by round per WC group
  const groups = [...new Set(mergedMatches.filter(m => m.stage === "group").map(m => m.group).filter(Boolean))] as string[];
  const groupMatchesByRound: Record<string, typeof mergedMatches[]> = {};
  groups.forEach((g) => {
    const gMatches = mergedMatches.filter((m) => m.stage === "group" && m.group === g).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
  ...
  function getPointsForMatches(participant: Participant, matchIds: string[]): number {
    return matchIds.reduce((sum, matchId) => {
      ...
      const bet = mergedBets.find((b) => b.matchId === matchId && b.participantId === participant.id);
  ...
  const hasConfrontations = Object.keys(confrontations).length > 0;

  // Knockout rounds info
  const knockoutRounds = [
    ...
    ...round,
    matchIds: mergedMatches.filter((m) => round.wcStages.includes(m.stage)).map((m) => m.id),
  }));
    ...
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Standings e confrontos calculados com base nos resultados simulados
        </p>
      </div>
        ...
        <>
          ...
          {hasConfrontations && (
            <div className="space-y-4">
              <h2 className="font-heading text-lg font-semibold">Fase de Grupos — Confrontos Simulados</h2>
              <Tabs defaultValue="1" className="w-full">
                ...
                {[1, 2, 3].map((round) => (
                  <TabsContent key={round} value={String(round)} className="mt-4">
                    <p className="text-xs text-muted-foreground mb-4">Pontos baseados nos palpites simulados da {round}ª rodada da fase de grupos</p>
                    <div className="grid md:grid-cols-2 gap-6">
